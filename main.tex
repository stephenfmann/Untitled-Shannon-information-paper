...
It is this that I take to be orthodoxy, and it is the justification of this kind of claim that I argue against here.\footnote{A commentator urged me to distinguish between two sceptical views: a strong view which denies Shannon information or closely related ideas are relevant \textit{at all} to understanding content in cognitive systems, and a weaker view which grants the relevance of Shannon information with respect to considerations of effective coding schemes, but denies that Shannon information has anything to say about the \textit{content} of representations in cognitive systems. I wish to explicitly clarify: nobody (as far as I know) holds the strong view, because everybody accepts the legitimate use of at least some information theoretic tools in cognitive science. In contrast, very many people hold the weak view. It is therefore the weak view that I am arguing against in this paper, especially its implication that efficient coding schemes can be discussed without reference to content.}
...
It must be the case that signals in the central model bear some exploitable relation to source strings, because the decoder is using the signals to reconstruct the original strings.
...
For the avoidance of doubt, I agree with this premise, and suggest encapsulating it as follows:
...
Burge seems happy that the apparent dispute can be read as terminological:

\begin{myquote}
There is nothing wrong with a broad notion of differential sensitivity and responsiveness that is associated with function. Such a notion describes the sensory capacities of many organisms, including the sensitivities of plants. One can use any term to express such a notion that one likes, including ‘representation'. Many biologists and psychologists, and some philosophers, do use the term ‘representation' in this way.
\par\hspace*{\fill}\citet[293]{burge2010origins}
\end{myquote}

What Burge demands is that there is a difference in \textit{kind}, rather than degree, between these simple states and genuine representation.
He argues that teleosemantics, by employing the term `representation' liberally, implies that there is no such difference in kind; that the differences between sensory registration and genuine representation are a matter of degree.
...
First, since scepticism has up until now been motivated by the two justifications I have spent the bulk of the paper dismantling, there's little reason left to cling to it.

Second, I admit there are aspects of genuine representation that just are not present in the central model (e.g. no perceptual constancies, nothing offline).
However, it's difficult to see how communication theory could \textit{fail} to extend to more sophisticated states.
Its mathematical core is about trading off effort for accuracy.
Genuine representations are physical entities, they are produced and maintained by a cognitive system, and they guide behaviour to be more or less successful depending on their accuracy.
It's difficult to motivate the idea that mathematics would simply \textit{stop describing that trade-off} at a certain level of sophistication, even when, as Burge demands, increased sophistication leads to a difference in kind rather than a difference in degree.
We are ignorant about whether communication theory could be extended to these more sophisticated systems because nobody has yet tried it.

Another author who offers a stronger notion of representation is Piccinini [CITE].
He distinguishes natural and nonnatural representation.
...
For this to form the basis of an objection to my claims, it would have to be the case that, first, philosophers were talking about nonnatural representation (or something like it) all along, and second, non-natural representation is outside the remit of communication theory.

It's fairly plausible that at least some philosophers had something like Piccinini's nonnatural representation in mind when imposing restrictions on the relevance of commuication theory for philosophical accounts of content.
One common refrain is that communication theory is about quantities of information while philosophy cares about informational content.
This goes naturally with the idea that communication theory is concerned with probabilities -- quantities -- while philosophy cares about content -- the aboutness of representation.

Let's accept that philosophers defending the orthodox view would align with Piccinini.
The second premise such an objection would require is that nonnatural representation, because it is construed in terms other than probability, lies outside the remit of communication theory.
...
And I have stressed that communication theory is at its most fundamental a collection of tools enabling a trade-off between the costs and benefits of representation.
I'm now going to suppose that these two points are accepted, and ask what this entails about nonnatural representation.

Since nonnatural representations are physical states (or processes), they will be subject to the same broad set of trade-offs as any other kind of representation.
(Piccinini's `natural representation' cuts across the signal/cue distinction; here I'm just talking about states with proper functions like signals and cognitive representations.)
Nonnatural representations cannot increase their accuracy for free: time and metabolic resources must be invested.
This is so even if their accuracy is not measured in terms of probability-raising.
The objection to my account requires that communication theory \textit{cannot} incorporate other notions of accuracy.
I find this implausible.
Suppose communication theory, as developed so far, has only considered representations that do their job by raising probabilities of states of the world.
And suppose there are representations that do their job in other ways (e.g. by bearing structural correspondences to world affairs), \textit{and} that these other ways cannot be captured in a probabilistic framework.
It would be true to say that communication theory as developed so far does not describe these other representations.
But would it be true to say communication theory \textit{cannot} describe them?
These other representations are still physical.
They still have a job to do, and must expend resources to do it.
Presumably there are mathematical relationships between the structural resemblance borne by such representations, the successful behaviour they engender, and the costs of constructing and maintaining them.

In sum, considerations of more sophisticated representations should motivate us to extend communication theory, not limit it.
