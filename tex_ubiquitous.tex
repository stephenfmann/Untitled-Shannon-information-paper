...
In failing to distinguish the theories, authors shorten the path to this further inference.
...
\begin{itemize}
    ...
    \item ``the Shannon–Weaver theory of communication'' \citep[p. 756 n. 3]{owren2010redefining}; ``Shannon-Weaver information theory'' \citep[p. 344; NB this work is not listed in the table]{dennett1983intentional}
    \item ``Shannon and Weaver's \parencite*{shannon1949mathematical} theory of information'' \citep[759]{owren2010redefining}; ``Shannon and Weaver's quantitative-information theory'' \citep[761]{owren2010redefining}
    \item ``Shannon–Weiner theory'' \citep[19]{baker2021natural}; beware both the typo (\textit{Weiner} instead of \textit{Wiener}) and the difference between Norbert Wiener and Warren Weaver
\end{itemize}
...
It must be noted that the four kinds of projects characterised above would be consistent with each other if only proponents agreed they meant different things by `semantic content'.
...
\citet{shea2018representation} defines representational content in a manner that does specific work in cognitive science; he explicitly disavows the claim that this notion should extend to other philosophical domains.
Much of the work that goes on under the rubric of figuring out what semantic content `really is' ends up delineating explanatorily interesting differences in the ways that vehicles can bear exploitable relations to the world.
Perhaps headway can be made by focusing more on the distinctions and less on their labels.
All that being said, I reiterate that the premise that is typically shared by scholars mentioned above is something I reject, and that this is not a purely verbal dispute -- however the debate over `semantic content' culminates.
