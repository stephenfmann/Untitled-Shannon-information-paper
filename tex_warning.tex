...
At its broadest, communication theory is a collection of tools and methods enabling an optimal trade-off between communicative effort and the benefits of accurate string reconstruction.
...
That is because minimising communicative effort is here equivalent to minimising the number of code symbols transmitted, on average.
...
This measure is more commonly known as \textbf{surprisal}, and is defined as $\log{\frac{1}{p(x)}}$ for a message $x$ that is produced by the source with probability $p(x)$.
...
One need only find a meaningful sentence and a nonsensical sentence that have the same probability of being produced at the source.
...
Describing Morse code, \citet[385]{shannon1948mathematicala} says that letters are ``represented by'' sequences of dots and dashes; in their widely cited textbook \citet[105]{cover2006elements} say the same.
...
Nevertheless, their usage is evidence that such a notion of content is required to make sense of the theorem.
...
Similarly, \citet[397]{shannon1948mathematicala} speaks of ``the number of bits [code symbols] required to specify the sequence'' of outcomes of the source process.
