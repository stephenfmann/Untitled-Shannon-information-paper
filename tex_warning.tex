...
Communication theory is geared towards improving efficiency.
...
One might think that the meanings of source strings \emph{could} play a role in solving this problem.
...
But Weaver's slight misquote led eventually to widespread misinterpretation.
...
% In order to derive a conclusion that concerns the relevance of informational measures for the meanings of signals, it is necessary to first derive a conclusion about the relevance of informational measures for meanings in general.
% That is how the argument above proceeds.

% I will challenge the argument by rejecting (P3).

% Bar-Hillel and Carnap thought that a philosophical theory would deliver the content of source strings in the central model.
% The problem is that \textit{no} theory could deliver that result.
% That is because the model does not provide enough detail for a theory to determine what their meanings could be.
% %
% % In addition, I will argue that the contents of signals are a crucial part of communication theory (section \ref{subsec:signalContent}).
% % The content of a signal in the central model -- a code string -- is just the source string from which it was encoded.
% % This is the result teleosemantics delivers, in agreement with textbook presentations of communication theory.
% %
% % (P3) asserted that the theory of semantic information sought by philosophers would deliver the meanings of (among other things) source strings in the central model.
% % How could such a theory have this result?
% As defined in that model, source strings are concatenations of symbols.
% Symbols themselves are formal tokens distinguished only by their relative probabilities.
% The central model does not include attributes that would allow a semantic theory to identify the alphabet from which those symbols are taken, nor the language in which those strings should be interpreted.
% No semantic theory could deliver the meanings of such strings.
...
% In order for a semantic theory to deliver the content of a string, it would have to know at least that its symbols are from the Latin alphabet specifically.
