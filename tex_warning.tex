...
The goal is achieved by \textbf{encoding} the source string, which means converting it into a sequence of physical events (typically electrical pulses) that can be transmitted from the source to the target.
The electrical pulses can then be decoded, and communication is deemed successful when the resultant target string is sufficiently similar to the source string.
...
Given an encoding scheme, any string of Latin symbols can be converted into a code string of 0s and 1s, and thus transmitted as a signal across a wire.
The decoder is faced with a line-up of Latin symbols, from which it must pick out the right symbols in the right order; the signal enables it to perform this task successfully.

It is costly to transmit encoded symbols.
Electric wires require power and time to operate.
At its broadest, communication theory is a collection of tools and methods enabling an optimal trade-off between signalling effort and the benefits of accurate string reconstruction.
...
That is because minimising signalling effort is here equivalent to minimising the number of code symbols transmitted, on average.
...
By `information' he means a certain property of source strings, defined as the number of symbols in the corresponding code string assuming an optimal encoding scheme.
...
It is clear that two messages -- two source strings -- can have the same surprisal, with one being a sentence of a natural language and the other being nonsense.
...
\noindent {\sc Shannon's Warning} is salient in a system with two lexicons.
...
\noindent The job of philosophers, as Bar-Hillel and Carnap saw it, was to provide a theory that would capture the aspect Shannon ignored.
Their goal was to develop a \textit{semantic theory of information}, characterised as follows:
...
\par\hspace*{\fill}\citet[148]{bar-hillel1953semantic}
\end{myquote}
}
...
By now, it is possible to cite the distinction between Shannon and semantic information, as distinct properties of signals, without further argument: ``I will interpret ‘information’ as ‘semantic information’ (i.e. semantic content), not as Shannon information'' \citep[p. 12 n. 14]{artiga2020signals}; see also \citet[6]{cao2020new} and \citet[1]{kolchinsky2018semantic}.
Weaver's use of the term `information' as synonymous with `surprisal' referred to a property of source strings, not signals.
...
Perhaps the collective misconstrual would not matter if not for the fact that the mutated form of the claim is false by the lights of communication theorists themselves.
It is also false by the lights of a popular theory of content.
...
Contemporary opponents of the relevance of communication theory for philosophical approaches to content focus on signals.
But nothing about signals can be concluded directly from {\sc Shannon's Warning}, which concerns source strings only.
The mathematical tools of communication theory are indeed blind to the meaning of a source string (if it even has one), but interpretations of the theory require that signals are meaningful: the content of a signal is the identity of a source string.
...
Describing Morse code, \citet[385]{shannon1948mathematicala} says that letters are ``represented by'' sequences of dots and dashes.
...
Nevertheless, their usage is evidence of a pretheoretic notion of representation at work -- perhaps something as weak as denotation or indication, but clearly a relation of some kind linking signals with source strings.
...
\noindent The extent to which their use of these loaded terms corresponds with the philosophers' notion of semantic content has not to my knowledge been asked often.
% Obviously the authors cannot be interpreted as taking a stance on philosophical theories of content.
Again, their usage is evidence that such a notion of content is required to make sense of the theorem.
...
Similarly, in a discussion of a special case of the theorem, \citet[397]{shannon1948mathematicala} speaks of ``the number of bits [code symbols] required to specify the sequence'' of outcomes of the source process.
...
So communication theorists from Shannon to contemporary textbook authors make use of a notion of semantic content, as indicated by their use of words like `represent', `describe' and `specify'.
Teleosemantics firms up their pretheoretic usage.
To see how, first note that it must intuitively be the case that signals in the central model bear some exploitable relation to source strings.
...
% The signal must bear a relation to the source string in order for the decoder to perform its function successfully.
